...
parser.add_argument('-ntomp', "--threads", required=True, type=int, help='Provide the number of logical processor')
parser.add_argument('-npme', "--ranks", required=False, default=None, type=int, help='Provide the number of PME ranks')
parser.add_argument('-ntomp_pme', "--pme_threads", required=False, default=None, type=int, help='Provide the numberof OMP threads per PME ranks')
...
parser.add_argument('-nb', "--nb", required=False, default=None, type=str, help='turn gpu on or off')
parser.add_argument('-gmx', "--gmx_path", required=False, default='gmx', type=str, help='Add full gromacs path')
...
for file in files:
    with open(file) as f:
        ...
        args = functions.replace_parameter("-nt", args, "-ntomp", str(threads))
        if npme is None:
            args = functions.remove_parameter("-ntmpi", args)
        if npme is not None:
            args = functions.replace_parameter("-ntmpi", args, "-npme", str(npme))
            args = functions.add_parameter("-ntomp_pme", args, str(threads_pme))
        if dynamic is not None:
            args = functions.add_parameter("-dlb", args, str(dynamic))
        if gpu_toggle is not None:
            args = functions.add_parameter("-nb", args, str(gpu_toggle))
